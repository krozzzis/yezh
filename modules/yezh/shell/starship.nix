{ delib, lib, pkgs, ... }:
delib.module {
  name = "yezh.shell.starship";

  options = { myconfig, ... }: {
    yezh.shell.starship.enable = delib.boolOption myconfig.user.shell.enable;
    yezh.shell.starship.useNerdFonts = delib.description (delib.boolOption true) "Enable Nerd Font icons for modules";
  };

  myconfig.ifEnabled = { myconfig, ... }:
  let
    cfg = myconfig.yezh.shell.starship;
  in
