{ delib, lib, pkgs, ... }:
delib.module {
  name = "yezh.media.reaper";

  options = { myconfig, ... }: {
    yezh.media.reaper.enable = delib.boolOption myconfig.user.gui.enable;
    yezh.media.reaper.quantum = delib.description (delib.intOption 256) "JACK quantum (buffer size in frames) for Reaper";
    yezh.media.reaper.rate = delib.description (delib.intOption 96000) "Sample rate for Reaper";
  };
