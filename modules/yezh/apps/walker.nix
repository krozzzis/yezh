...
delib.module {
  name = "yezh.apps.walker";

  options = { myconfig, ... }: {
    yezh.apps.walker.enable = delib.boolOption false;
    yezh.apps.walker.pkg = delib.packageOption (inputs.walker.packages.${pkgs.stdenv.hostPlatform.system}.default or pkgs.walker);
  };
  ...
  nixos.ifEnabled = { myconfig, ... }: {
    system.activationScripts.elephant = {
      text = ''
        su - ${myconfig.user.constants.username} -c 'rm -f "$HOME/.cache/walker/applications.json" 2>/dev/null; systemctl --user try-restart elephant.service 2>/dev/null' || true
      '';
      deps = [ "users" ];
    };
  };
